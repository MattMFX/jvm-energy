package dev.matheus.energy.jrapl;
...
public class EnergyCheckUtils {
    ...
    public static String getEnergyStats() {
        ...
        if (stats.length >= 3) {
            ...
            // Format without commas to avoid parsing issues
            return String.format(java.util.Locale.US, "%.9f,%.9f,%.9f", stats[2], stats[0], stats[1]);
        } else {
            return "0.0,0.0,0.0";
        }
    }
    ...
    public static double[] getEnergyStatsArray() {
        ...
        if(socketNum == 1) {
            ...
            // Convert European decimal format (comma) to US format (period) before parsing
            stats[0] = Double.parseDouble(energy[0].replace(',', '.'));
        ...
        /*Multiple sockets*/
            ...
            for(int i = 0; i < perSockEner.length; i++) {
                ...
                for(int j = 0; j < energy.length; j++) {
                    ...
                    // Convert European decimal format (comma) to US format (period) before parsing
                    stats[count] = Double.parseDouble(energy[j].replace(',', '.'));
                }
            }
            return stats;
        }
